...
function compute_bonus_average($cid, $entry)
{
      $sql = "SELECT * from bonuspoints where entry=$entry";
...
$sql = "select id, entry from bonus where groupnum=$groupnum";
echo $sql;
$submissions = mysqli_query($cid, $sql);

while ($submission = mysqli_fetch_array($submissions, MYSQLI_ASSOC))
{
  $avg = compute_bonus_average($cid, $submission["entry"]);
  $id = $submission["id"];
  ...
  $sql = "update points set presentation=$cp where entry=$entry";
  echo $sql . "\n";
